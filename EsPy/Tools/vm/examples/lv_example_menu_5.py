...
create_slider(section,lv.SYMBOL.SETTINGS, "速度", 0, 150, 120)
create_slider(section,lv.SYMBOL.SETTINGS, "加速", 0, 150, 50)
create_slider(section,lv.SYMBOL.SETTINGS, "重力", 0, 150, 80)
...
create_switch(section,lv.SYMBOL.AUDIO, "声音", False)
...
create_slider(section,lv.SYMBOL.SETTINGS, "智能", 0, 150, 100)
...
create_text(section,None, "版本号 1.0", LV_MENU_ITEM_BUILDER_VARIANT_1)
...
# Create a root page
root_page = lv.menu_page(menu, "设置")
...
menu.set_style_text_font(font_simsun_st12, 0)
cont = create_text(section, lv.SYMBOL.AUDIO, "声音", LV_MENU_ITEM_BUILDER_VARIANT_1);
menu.set_load_page_event(cont, sub_sound_page)
cont = create_text(section, lv.SYMBOL.SETTINGS, "显示", LV_MENU_ITEM_BUILDER_VARIANT_1);
menu.set_load_page_event(cont, sub_display_page)

create_text(root_page, None, "其它", LV_MENU_ITEM_BUILDER_VARIANT_1);
section = lv.menu_section(root_page)
cont = create_text(section, None, "关于", LV_MENU_ITEM_BUILDER_VARIANT_1);
menu.set_load_page_event(cont, sub_about_page)
cont = create_text(section, lv.SYMBOL.SETTINGS, "菜单模式", LV_MENU_ITEM_BUILDER_VARIANT_1);
